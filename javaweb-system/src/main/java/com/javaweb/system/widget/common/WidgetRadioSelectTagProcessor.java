...
public class WidgetRadioSelectTagProcessor extends AbstractElementTagProcessor {
    /**
     * 标签名
     */
     ...
     */
    public WidgetRadioSelectTagProcessor(String dialectPrefix) {
        super(
                // 此处理器将仅应用于HTML模式
                TemplateMode.HTML,
                // 要应用于名称的匹配前缀
        ...
        if (!StringUtils.isEmpty(tagSql)) {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    if (tagValue.contains(id)) {
                        model.add(modelFactory.createOpenElementTag(String.format("input type=\"radio\" name=\"%s\" value=\"%s\" title=\"%s\" checked=\"\"", showName, id, name)));
                    } else {
                        model.add(modelFactory.createOpenElementTag(String.format("input type=\"radio\" name=\"%s\" value=\"%s\" title=\"%s\"", showName, id, name)));
                    }
                }
            ...
            for (Map.Entry<Integer, String> item : map.entrySet()) {
                if (tagValue.contains(item.getKey().toString())) {
                    model.add(modelFactory.createOpenElementTag(String.format("input type=\"radio\" name=\"%s\" value=\"%s\" title=\"%s\" checked=\"\"", showName, item.getKey(), item.getValue())));
                } else {
                    model.add(modelFactory.createOpenElementTag(String.format("input type=\"radio\" name=\"%s\" value=\"%s\" title=\"%s\"", showName, item.getKey(), item.getValue())));
                }
            }
        }
        // 利用引擎替换整合标签
